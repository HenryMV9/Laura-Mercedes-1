import{a as f}from"./cart-DaXgv_wZ.js";/* empty css             */import{g as v,a as h}from"./supabase-client-D2vQk3vi.js";import{i as k,c as E,f as u}from"./currency-selector-D4CMqF52.js";let l=[],r=[],n={categoryId:null,minPrice:null,maxPrice:null,inStock:!1};const I=new URLSearchParams(window.location.search),g=I.get("category");async function B(){const a=await v(),c=document.getElementById("categoryFilters");if(c.innerHTML=`
        ...
        `).join("")}
      `,g){const e=a.find(t=>t.slug===g);if(e){n.categoryId=e.id;const t=c.querySelector(`input[value="${e.id}"]`);t&&(t.checked=!0)}}}async function P(){l=await h(),s()}function s(){r=l.filter(a=>!(n.categoryId&&a.category_id!==n.categoryId||n.minPrice&&a.price<n.minPrice||n.maxPrice&&a.price>n.maxPrice||n.inStock&&!a.in_stock)),o()}function o(){const a=document.getElementById("productsGrid"),c=document.getElementById("resultsCount");if(c.textContent=`${r.length} ${r.length===1?"product":"products"}`,r.length===0){a.innerHTML=`
          ...
          <div class="product-card">
            ...
            <div class="product-info">
              ...
              <div class="product-price">
                <span class="current-price">${u(e.price)}</span>
                ${t?`<span class="original-price">${u(e.compare_at_price)}</span>`:""}
              </div>
            </div>
          </div>
        `}).join(""),document.querySelectorAll(".product-quick-add").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const i=e.dataset.id,m=l.find(y=>y.id===i);m&&(f(m),e.textContent="Added!",setTimeout(()=>{e.textContent="Add to Cart"},2e3))})})}function p(){n={categoryId:null,minPrice:null,maxPrice:null,inStock:!1},document.querySelector('input[name="category"][value=""]').checked=!0,document.getElementById("minPrice").value="",document.getElementById("maxPrice").value="",document.getElementById("inStockFilter").checked=!1,s()}document.getElementById("applyFilters").addEventListener("click",()=>{const a=document.querySelector('input[name="category"]:checked')?.value;n.categoryId=a||null,n.minPrice=parseFloat(document.getElementById("minPrice").value)||null,n.maxPrice=parseFloat(document.getElementById("maxPrice").value)||null,n.inStock=document.getElementById("inStockFilter").checked,s(),window.innerWidth<=1024&&d()});document.getElementById("clearFilters").addEventListener("click",p);document.getElementById("sortSelect").addEventListener("change",a=>{switch(a.target.value){case"price-low":r.sort((e,t)=>e.price-t.price);break;case"price-high":r.sort((e,t)=>t.price-e.price);break;case"name":r.sort((e,t)=>e.name.localeCompare(t.name));break;default:r.sort((e,t)=>new Date(t.created_at)-new Date(e.created_at))}o()});document.getElementById("mobileFilterToggle").addEventListener("click",()=>{document.getElementById("shopFilters").classList.add("active"),document.getElementById("filtersBackdrop").classList.add("active"),document.getElementById("filterClose").style.display="block",document.body.style.overflow="hidden"});function d(){document.getElementById("shopFilters").classList.remove("active"),document.getElementById("filtersBackdrop").classList.remove("active"),document.body.style.overflow=""}document.getElementById("filterClose").addEventListener("click",d);document.getElementById("filtersBackdrop").addEventListener("click",d);await k();const C=E(a=>{o()});document.getElementById("currencySelector").appendChild(C);await B();await P();
